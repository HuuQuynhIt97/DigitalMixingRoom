...
namespace ScalingMachineService
{
    public class Worker2 : BackgroundService
    {
        ...
        public Worker2(ILogger<Worker2> logger, AppSettings appSettings)
        {
            _logger = logger;
        ...
        {
            _logger.LogInformation("Worker2 Start: {time}", DateTimeOffset.Now);
            await base.StartAsync(cancellationToken);
        }

        public override async Task StopAsync(CancellationToken cancellationToken)
        {
            _logger.LogInformation("Worker2 Stop: {time}", DateTimeOffset.Now);
            ...
            try
            {
                ...
                if (message.Trim() != string.Empty)
                {
                    ...
                    if (time >= _appSettings.CycleTime)
                    {
                        ...
                        try
                        {
                            _connection.InvokeAsync("Welcom", _appSettings.MachineID.ToString(), messages.ToString(), _appSettings.MachineID == 1 ? "k" : "g");
                        }
                        catch (Exception ex)
                        {
                            _logger.LogError(ex.Message);
