...
export class StirComponent implements OnInit, OnDestroy {
  ...
  ngOnDestroy(): void {
    ...
    this.stirService.numberOfAttempts = 5;
    this.stirService.close().then((result) => {
      console.log('Stir service stopped connection');
    }).catch((err) => {
      console.log('Stir service can not stopped connection', err);
    });
  }
  public ngOnInit(): void {
    this.stirService.connect();
  ...
  async confirmData(item) {
    ...
    try {
      ...
      await this.update(stirModel as IStirForUpdate) as IStir;
      // const data = await this.getStirByMixingInfoID(this.mixingInfoID);
      // this.stirData = data;
      this.getStandardRPMAndDuration();
      this.goToTodolist();
