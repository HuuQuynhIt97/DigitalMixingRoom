...
@Component({
  selector: 'app-report-done',
  templateUrl: './report-done.component.html',
  styleUrls: ['./report-done.component.scss']
})
export class ReportDoneComponent extends BaseComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.pageSettings = { pageCount: 5, pageSizes: ['All', 20,50,100,200], pageSize: 50 };
  ...
  consumptionByLineCase1() {
    this.spinner.show()
    this.planService.consumptionReportDone(this.buildingID, this.startDate, this.endDate).subscribe((res: Consumtion[]) => {
      this.data = res;
      this.spinner.hide()
    });
  }
  ...
  reportConsumptionCase1() {
    ...
    // }
    this.spinner.show();
    this.planService.reportDoneList(this.buildingID, this.startDate, this.endDate).subscribe((data: any) => {
      ...
      link.download = `${startDateFormat}-${endDateFormat}_Report-Done.xlsx`;
      link.click();
      this.spinner.hide();
    });
  }
