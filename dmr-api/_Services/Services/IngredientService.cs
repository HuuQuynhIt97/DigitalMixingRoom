    ...
    public class IngredientService : IIngredientService
    {
        ...
        public async Task<object> ScanQRCodeFromChemialWareHouseV1(ScanQrCodeDto entity)
        {
            ...
            if (await _repoIngredientInfoReport.CheckBarCodeExists(partNo))
            {
                ...
                if (result != null)
                {
                    result.Qty = model.Unit + result.Qty;
        ...
        public async Task<object> ScanQRCodeFromChemialWareHouse(string qrCode, string building, int userid)
        {
            ...
            {
                // check tiep trong bang ingredientReport xem co du lieu chua 
                var result = _repoIngredientInfoReport.FindAll()
                    .FirstOrDefault(
                    x => x.Code == Barcode 
                    && x.Batch == Batch 
                    && x.CreatedDate.Date == currentDate.Date
                    );
        ...
        public async Task<bool> DeleteIngredientInfo(int id, string code, double qty, string batch)
        {
        ...
        public async Task<bool> Update(string code, double qty, string batch)
        {
            try
            {
        ...
        public async Task<ResponseDetail<object>> Rate()
        {
            ...
            var complete = model.Where(
                x => x.MaterialNO != "" 
                && x.VOC > 0 
                && x.ExpiredTime > 0 
                && x.DaysToExpiration > 0 
                && x.Unit > 0).Count();
