...
using static DMR_API.Helpers.CodeUtility;
       ...
       public async Task<MixingInfo> AddMixingInfoAsync(MixingInfoForAddDto mixing)
        {
            using var transaction = await _unitOfWork.BeginTransactionAsync();
            {
                try
                {
                    ...
                    //find chemical position = A
                    var positionA = mixing.Details.First(o => o.Position == "A");
                    var expiredTimeA = _repoIngredient.FindById(positionA.IngredientID).ExpiredTime;
                    var checmicalA = glue.GlueIngredients.FirstOrDefault().Ingredient.ExpiredTime;
                    mixingInfo.CreatedTime = DateTime.Now;
                    mixingInfo.ExpiredTime = DateTime.Now.AddHours(expiredTimeA);
                    string code = RandomString(8);
                    while (true)
                    {
