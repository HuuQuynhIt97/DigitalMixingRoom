...
namespace DMR_API._Services.Services
{
    public class PlanService : IPlanService
    {
        ...
        // report
        public async Task<Byte[]> ExportExcelDoneList(ReportParams reportParams)
        {
            ...
            var model = await _repoToDoList.FindAll(x =>
                   x.IsDelete == false
                   && (x.EstimatedStartTime.Date >= reportParams.StartDate.Date && x.EstimatedStartTime.Year == reportParams.StartDate.Year)
                   && (x.EstimatedFinishTime.Date <= reportParams.EndDate.Date && x.EstimatedFinishTime.Year == reportParams.EndDate.Year)
                   && x.PrintTime != null //Thêm bởi Quỳnh (1/28/2021 11:46)
                   && x.BuildingID == reportParams.BuildingID
                   )
                .Include(x => x.Plan)
            ...
            var delay = await _repoToDoList.FindAll(x =>
               ...
               .Select(x => new
               {
                   ...
                   ModelNO = x.Plan.BPFCEstablish.ModelNo == null ? "N/A" : x.Plan.BPFCEstablish.ModelNo.Name,
                   ArticleNO = x.Plan.BPFCEstablish.ArticleNo == null ? "N/A" : x.Plan.BPFCEstablish.ArticleNo.Name,
               }).Where(x => x.GlueName.Contains(" + ")) //Thêm bởi Quỳnh (1/28/2021 11:46)
              .ToListAsync();
            ...
            var delayDispatchListModel = await _repoDispatchList.FindAll(x =>
                 ...
                 && (x.EstimatedFinishTime.Date <= reportParams.EndDate.Date && x.EstimatedFinishTime.Year == reportParams.EndDate.Year)
                 && x.EstimatedFinishTime < currentTime
                 && x.FinishDispatchingTime == null //Thêm bởi Quỳnh (1/28/2021 11:46)
                 && x.BuildingID == reportParams.BuildingID
                 )
              .Include(x => x.Plan)
                  .ThenInclude(x => x.BPFCEstablish)
            ...
            var groupByDispatchList = dispatchListModel.Concat(delayDispatchListModel).GroupBy(x => new { x.EstimatedStartTime, x.GlueName })
              .OrderBy(x => x.Key.EstimatedStartTime).ThenBy(x => x.Key.GlueName).ToList();
            ...
            try
            {
                ...
                using (ExcelPackage p = new ExcelPackage(memoryStream))
                {
                    ...
                    foreach (var bodyItem in model)
                    {
                        ...
                        if (bodyItem.EstimatedFinishTime < currentTime && bodyItem.PrintTime == null)  //Thêm bởi Quỳnh (1/28/2021 11:46)
                        {
                            SMTExcelRange.Value = "-";
                        }
                        else
                        ...
                        {
                            StdConExcelRange.Value = $"{Math.Round(bodyItem.StandardConsumption, 2)}";  //update bởi Quỳnh (1/28/2021 11:46)
                        }
                        ...
                        {
                            MixedConExcelRange.Value = !bodyItem.GlueName.Contains(" + ") ? "N/A" : $"{Math.Round(bodyItem.MixedConsumption, 2)}"; //update bởi Quỳnh (1/28/2021 11:46)
                        }
                        ...
                        if (bodyItem.EstimatedFinishTime < currentTime && bodyItem.PrintTime == null) //update bởi Quỳnh (1/28/2021 11:46)
                        {
        ...
        public async Task<bool> CreateForStopLine(PlanDto model)
        {
            ...
            try
            {
                foreach (var item in model.listAdd)
                {
                    ...
                    var startTime = item.DueDate.Date.Add(new TimeSpan(07, 30, 0)).ToRemoveSecond();
                    var endTime = item.DueDate.Date.Add(new TimeSpan(16, 30, 0)).ToRemoveSecond();
        ...
        public async Task<ToDoListForReturnDto> Done(ReportParams reportParams)
        {
           ...
           .ToListAsync();
            ...
            var lines = await _repoBuildingUser.FindAll().Include(x => x.Building).Where(x => x.Building.ParentID == reportParams.BuildingID).Select(x => x.BuildingID).ToListAsync();
            if (lines.Count > 0)
            {
            ...
            {
                var allLines = await _repoBuilding.FindAll().Where(x => x.ParentID == reportParams.BuildingID).Select(x => x.ID).ToListAsync();
        ...
        public async Task<object> ReportDoneList(ReportParams reportParams)
        {
            var res = await Done(reportParams);
