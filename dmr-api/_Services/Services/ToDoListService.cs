    ...
    public class ToDoListService : IToDoListService
    {
        ...
        public async Task<ToDoListForReturnDto> Done(int buildingID)
        {
           ...
           .ToListAsync();
            ...
            var lines = await _repoBuildingUser.FindAll(o => o.UserID == userID).Include(x => x.Building).Where(x => x.Building.ParentID == buildingID).Select(x => x.BuildingID).ToListAsync();
            if (lines.Count > 0)
            {
